...
// ... inside your existing message handler
...
if (command === "reviewpanel") {
    const reviewPanelEmbed = new EmbedBuilder()
        ...
        .setDescription(
            "We highly value your feedback and kindly request your participation in a brief review to assist us in enhancing our services."
        )
        .setColor(0x2b2d31)
        .setFooter({
            text: `Request created at: ${new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} AM - All rights reserved to ReviewHub`,
    ...
    const reviewPanelRow = new ActionRowBuilder().addComponents(submitReviewButton);

    await message.channel.send({ embeds: [reviewPanelEmbed], components: [reviewPanelRow] });
}

// ... in your interaction handler (Events.InteractionCreate)

if (interaction.isButton() && interaction.customId === "submit_review_modal") {
...
if (interaction.isModalSubmit() && interaction.customId === "review_modal") {
    ...
    const reviewEmbed = new EmbedBuilder()
        ...
        .setDescription(`**${interaction.user.username}** • new review\n\n${reviewMessage}`)
        .addFields(
            { name: "Rating", value: `${starsDisplay} (${rating}/5)`, inline: true },
    ...
    // Action buttons (Report, Submit A Review, Useful)
    ...
    const ownerResponseEmbed = new EmbedBuilder()
        .setDescription(`**Owner's Response** See Thread >\nThere are no messages in this thread yet.`)
        .setColor(0x2b2d31);
...
// ... handle report_*, useful_* etc. button logic as you currently do
    // 🛠️ Help Command
    if (command === "help") {
        const embed = new EmbedBuilder()
